/*
 * Sipher — Privacy-as-a-Skill API
 *
 * REST API wrapping SIP Protocol's privacy SDK for Solana agents. Stealth addresses, Pedersen commitments, shielded transfers, and viewing key compliance.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: hello@sip-protocol.org
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct GetRpcProviders200ResponseData {
    #[serde(rename = "active", skip_serializing_if = "Option::is_none")]
    pub active: Option<Box<models::GetRpcProviders200ResponseDataActive>>,
    #[serde(rename = "supported", skip_serializing_if = "Option::is_none")]
    pub supported: Option<Vec<models::GetRpcProviders200ResponseDataSupportedInner>>,
}

impl GetRpcProviders200ResponseData {
    pub fn new() -> GetRpcProviders200ResponseData {
        GetRpcProviders200ResponseData {
            active: None,
            supported: None,
        }
    }
}

